...
<head>
    ...
    <title>Laporan Semua Pendaftar</title>
    <style>
        body {
...
<body>
    <div class="header">
        ...
        @endif
        <h2>LAPORAN SEMUA PENDAFTAR</h2>
        <h3>SISTEM PENERIMAAN MURID BARU (SPMB)</h3>
        <p>Jadwal Aktif: {{ $jadwalAktif?->nama_jadwal ?? 'Tidak Ada Jadwal Aktif' }}</p>
    </div>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $user?->status_pendaftaran ? Str::title(str_replace('_', ' ', $user->status_pendaftaran)) :
                        'Belum Diverifikasi' }}</td>
                    <td>{{ $user?->biodata?->asal_sekolah ?? '-' }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" class="text-center">Tidak ada data pendaftar tersedia.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="footer">
        <p>Laporan Pendaftar SPMB - Dicetak oleh sistem pada {{ now()->translatedFormat('d F Y H:i:s') }}</p>
    </div>
</body>
