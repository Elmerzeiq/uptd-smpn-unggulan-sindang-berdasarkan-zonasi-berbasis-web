...
<head>
    ...
    <title>Laporan Status Berkas</title>
    <style>
        body {
...
<body>
    <div class="header">
        ...
        @endif
        <h2>LAPORAN STATUS BERKAS PENDAFTAR</h2>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $user?->berkas?->status_verifikasi ?? 'Belum Upload' }}</td>
                    <td>{{ $user?->berkas?->catatan_admin ?? '-' }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="text-center">Tidak ada data berkas tersedia.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="footer">
        <p>Laporan Status Berkas SPMB - Dicetak oleh sistem pada {{ now()->translatedFormat('d F Y H:i:s') }}</p>
    </div>
</body>
